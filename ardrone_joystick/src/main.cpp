...
struct TeleopArDrone
{
  ...
  void joyCb(const sensor_msgs::JoyConstPtr joy_msg) {
    ...
    for (int i = 0; i < joy_msg->buttons.size(); ++i) {
      ROS_INFO("Button %zu: %zu", i, joy_msg->buttons.at(i));
    }
    ROS_INFO("");
    ...
    if (!isApprox(twist.linear.x, 0.0, 1e-10) ||
        !isApprox(twist.linear.y, 0.0, 1e-10) ||
        !isApprox(twist.linear.z, 0.0, 1e-10) ||
        !isApprox(twist.angular.z, 0.0, 1e-10)) {
      autopilot_ = false;
    }
